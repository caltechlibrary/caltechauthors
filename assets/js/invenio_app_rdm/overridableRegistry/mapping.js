...
const DepositForm = (props) => {
    ...
    const { preselectedCommunity } = props;
    const { filesLocked } = props;
    const { recordRestrictionGracePeriod } = props;
    const { allowRecordRestriction } = props;
    const { groupsEnabled } = props;
    const { allowEmptyFiles } = props;
    const { customFieldsUI } = props;
...
export const overriddenComponents = {"InvenioCommunities.RequestSearch.SearchHelpLinks":SearchHelpLinks,"InvenioAppRdm.Deposit.RDMDepositForm.layout":DepositForm }; 
